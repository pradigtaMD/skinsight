...
export default function KlinikPage() {
  ...
  const handleSubmitBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.status === "success") {
        // ==========================================
        // 2. PREMIUM ALERT: SUKSES (Durasi 7 Detik + Tombol Manual)
        // ==========================================
        let timeLeft = 7; // <--- Durasi diubah menjadi 7 detik
        ...
        Swal.fire({
          html: `
            <div class="flex flex-col items-center p-2">
              ...
              <div class="text-center px-4 w-full">
                ...
                <h3 class="font-serif text-4xl text-[#3A2E2B] leading-tight mb-6">Reservasi<br/><span class="italic font-normal">Berhasil!</span></h3>
                
                <div class="w-full h-[1px] bg-[#EAE3D9] mb-6"></div>
                
                <p class="text-sm text-[#8B736D] leading-relaxed mb-6">
                  Jadwal konsultasi eksklusif Anda bersama<br/> 
                  <b class="text-[#3A2E2B] text-base">${selectedDoctor.name}</b><br/>
                ...
                <div class="bg-gray-50 border border-[#EAE3D9] p-4 rounded-2xl w-full flex flex-col items-center gap-3">
                  <div class="flex items-center gap-2">
                    ...
                    <p class="text-[10px] text-[#8B736D] tracking-wider uppercase font-bold">
          ...
          allowOutsideClick: false,
          customClass: {
            popup: 'rounded-[3rem] border-2 border-[#660033]/5 shadow-[0_30px_80px_rgba(102,0,51,0.2)] pb-8 pt-6 overflow-visible w-full max-w-[360px] md:max-w-[400px]',
          ...
          // Aksi setelah 7 detik selesai: Reset form dan buka WA
        ...
        Swal.fire({
          ...
          buttonsStyling: false,
          customClass: {
            popup: 'rounded-[3rem] border border-red-100 shadow-2xl shadow-red-900/10 pb-8 pt-6',
      ...
      // Alert Gagal Koneksi tetap sederhana karena ini kondisi darurat
      Swal.fire({
  ...
  const handleSelectDoctor = (doc: Doctor) => {
    ...
    // Memberikan delay kecil agar render selesai sebelum scroll di mobile
    if (window.innerWidth < 1024) {
      setTimeout(() => {
        const formElement = document.getElementById('booking-form');
        if (formElement) {
          const yOffset = -80; // Penyesuaian agar tidak tertutup header/navbar
...
<main className="min-h-screen bg-[#FDFBF7] -mt-16 md:-mt-20 pt-28 md:pt-36 pb-28 md:pb-12">      
      ...
      <section className="px-5 md:px-10 max-w-7xl mx-auto mb-12 md:mb-16">
        <div className="text-center md:text-left mb-8 md:mb-10">
          ...
          <h1 className="text-4xl md:text-6xl text-[#660033] font-serif leading-tight mb-4" style={{ fontFamily: 'var(--font-playfair)' }}>
            Meet Our <span className="italic text-[#3A2E2B]">Specialists</span>
          </h1>
        </div>

        <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 border border-[#EAE3D9] shadow-xl shadow-[#660033]/5 flex flex-col lg:flex-row items-center gap-6 md:gap-8 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 h-48 md:w-64 md:h-64 bg-[#F8F3ED] rounded-full -mr-24 -mt-24 md:-mr-32 md:-mt-32 opacity-40"></div>
          
          <div className="flex-1 relative z-10 w-full">
            <div className="flex items-center justify-center md:justify-start gap-2 mb-4">
              <span className="w-10 h-[1px] bg-[#660033]"></span>
              <span className="text-[#660033] text-[10px] font-black uppercase tracking-widest">Pusat Estetika</span>
            </div>
            <h2 className="text-2xl md:text-3xl font-bold text-[#3A2E2B] mb-4 text-center md:text-left">Azzahra Skin Clinic</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="flex gap-3 items-start bg-gray-50/50 p-3 md:p-0 rounded-xl md:bg-transparent">
                <span className="text-xl">📍</span>
                <p className="text-[#8B736D] text-xs md:text-sm leading-relaxed">
                  Jl. Puebongo I, Boyaoge, Kec. Tatanga,<br /> Kota Palu, Sulawesi Tengah 94111
                </p>
              </div>
              <div className="flex gap-3 items-start bg-gray-50/50 p-3 md:p-0 rounded-xl md:bg-transparent">
                <span className="text-xl">🕒</span>
                <div>
                  <p className="text-[#3A2E2B] text-xs md:text-sm font-bold">Jam Operasional</p>
                  <p className="text-[#8B736D] text-xs">Setiap Hari (10.00 - 21.00 WITA)</p>
                </div>
              </div>
            </div>
          </div>

          <a href="https://maps.google.com/?q=Jl.+Puebongo+I,+Boyaoge,+Palu" target="_blank" rel="noreferrer" className="w-full lg:w-auto relative z-10 px-8 py-4 bg-[#660033] text-white rounded-2xl text-xs font-bold tracking-widest hover:bg-[#4A0024] transition-all text-center shadow-lg shadow-[#660033]/20 uppercase">
            LIHAT MAPS
          </a>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-10 mb-20">
        ...
        <div className="w-full mb-8 md:mb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl md:text-2xl font-serif font-bold text-[#660033]" style={{ fontFamily: 'var(--font-playfair)' }}>
              Physicians on Duty
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-12 items-start">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-8">
                ...
                return (
                  <div 
                    ...
                    className={`relative group rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 border-2 transition-all duration-500 flex flex-col items-center text-center 
                  ...
                  >
                    {/* Checkmark Card */}
                    <div className={`absolute -top-3 -right-3 w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-all duration-500 z-20 shadow-lg ${isSelected ? 'bg-[#660033] scale-100' : 'bg-gray-200 scale-0'}`}>
                    ...
                    {/* FOTO DOKTER & BADGE */}
                    <div className="relative w-24 h-24 md:w-32 md:h-32 mb-6 md:mb-8 mt-2">
                      ...
                      <img src={doc.image} className={`w-full h-full object-cover rounded-full p-1.5 md:p-2 ${!isAvailable && 'grayscale opacity-70'}`} alt={doc.name} />
                      
                      <div className={`absolute -bottom-2 md:-bottom-3 left-1/2 -translate-x-1/2 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full shadow-md border flex items-center gap-1.5 whitespace-nowrap z-10 ${!isAvailable ? 'bg-white border-red-100' : 'bg-white border-[#EAE3D9]'}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${isAvailable ? 'bg-[#25D366] animate-pulse' : 'bg-red-500'}`}></span>
                        <span className={`text-[7px] md:text-[8px] font-black uppercase tracking-widest ${isAvailable ? 'text-[#25D366]' : 'text-red-500'}`}>
                          {isAvailable ? 'Tersedia' : 'Off Duty'}
                        </span>
                      </div>
                    </div>

                    <h3 className={`font-bold text-lg md:text-xl mb-1 ${!isAvailable ? 'text-gray-400' : isSelected ? 'text-[#660033]' : 'text-[#3A2E2B]'}`}>{doc.name}</h3>
                    <p className={`text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.25em] font-black mb-5 md:mb-6 ${!isAvailable ? 'text-gray-300' : 'text-[#A58D87]'}`}>{doc.specialty}</p>

                    <div className="mt-auto w-full">
                      <div className={`w-full py-3 md:py-3.5 rounded-2xl font-black text-[9px] md:text-[10px] tracking-widest uppercase border-2 transition-all 
          ...
          <div id="booking-form" className="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-32 z-30 h-fit mt-4 lg:mt-0">
            <div className="bg-[#660033] text-[#FDFBF7] p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl shadow-[#660033]/30 border border-[#4A0024] relative overflow-hidden">
              <h2 className="text-2xl md:text-3xl font-serif mb-1" style={{ fontFamily: 'var(--font-playfair)' }}>Reservation</h2>
              <p className="text-[10px] md:text-xs text-white/70 mb-6 md:mb-8 uppercase tracking-widest">
                Consult with <span className="text-white font-bold">{selectedDoctor?.name || '...'}</span>
              </p>
              
              <form onSubmit={handleSubmitBooking} className="space-y-4 md:space-y-5">
                <div className="space-y-1.5">
                  <label className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-white/60 ml-1">Nama Pasien</label>
                  <input type="text" required value={formData.patient_name} onChange={(e) => setFormData({...formData, patient_name: e.target.value})} placeholder="Nama lengkap Anda" className="w-full px-4 py-3.5 md:py-4 rounded-2xl bg-white/10 border border-white/10 text-white outline-none text-xs md:text-sm placeholder:text-white/30" />
                </div>
                
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-white/60 ml-1">Tanggal</label>
                    <input type="date" required min={today} value={formData.booking_date} onChange={(e) => setFormData({...formData, booking_date: e.target.value})} className="w-full px-3 py-3.5 md:px-4 md:py-4 rounded-2xl bg-white/10 border border-white/10 text-white text-xs [color-scheme:dark]" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-white/60 ml-1">Waktu Sesi</label>
                    <select value={formData.session_time} onChange={(e) => setFormData({...formData, session_time: e.target.value})} className="w-full px-3 py-3.5 md:px-4 md:py-4 rounded-2xl bg-white/10 border border-white/10 text-white text-xs outline-none appearance-none">
                ...
                <div className="space-y-1.5">
                  <label className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-white/60 ml-1">Keluhan (Opsional)</label>
                  <textarea rows={2} value={formData.keluhan} onChange={(e) => setFormData({...formData, keluhan: e.target.value})} placeholder="Tuliskan kondisi kulit atau jenis perawatan..." className="w-full px-4 py-3.5 md:py-4 rounded-2xl bg-white/10 border border-white/10 text-white resize-none text-xs md:text-sm placeholder:text-white/30"></textarea>
                </div>
                
                <button 
                  type="submit" 
                  disabled={!selectedDoctor || isSubmitting} 
                  className={`w-full py-4 md:py-5 rounded-2xl font-black transition-all shadow-xl mt-2 text-[10px] md:text-[11px] tracking-[0.15em] md:tracking-[0.2em] uppercase active:scale-95 flex items-center justify-center gap-2.5 
      ...
      {/* 3. SECTION FAQ */}
      <section className="py-16 md:py-32 bg-[#660033] border-t-4 border-[#4A0024] relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-5 md:px-6 relative z-10">
          ...
          <div className="space-y-4 md:space-y-6">
            {[
              ...
              <div key={i} className="p-6 md:p-8 bg-white rounded-[1.5rem] md:rounded-[2.5rem] flex flex-col md:flex-row items-start gap-4 md:gap-5 shadow-2xl shadow-black/20 hover:scale-[1.01] transition-transform">
